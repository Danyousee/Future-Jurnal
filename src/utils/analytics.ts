...
export function calculateJournalStats(
  ...
  sortedTrades.forEach((trade, idx) => {
    ...
    let feeVal = (trade.makerFee || 0) + (trade.takerFee || 0);
    if (!feeVal) {
      if (trade.totalFees !== undefined && trade.totalFees > 0) {
        feeVal = trade.totalFees;
      } else if (trade.tradeMode === 'DEX' || (trade.tradeMode === 'SPOT' && trade.spotVenue === 'DEX')) {
        const gas = trade.dexGasFee || trade.gasFee || 0;
        const netFee = trade.dexNetworkFee || trade.networkFee || 0;
        const dexSwapFee = size * ((trade.dexTradingFeePct || trade.dexFeePct || 0.3) / 100);
        feeVal = gas + netFee + dexSwapFee;
      } else if (trade.tradeMode === 'FOREX') {
        feeVal = (trade.lotSize ? trade.lotSize * 7 : 0) || (size * feeRate);
      } else {
        feeVal = size * feeRate * 2;
      }
    }
    ...
    // Risk Tracking
    const risk = trade.riskAmount || (trade.riskPercentage && initialBalance ? (initialBalance * trade.riskPercentage) / 100 : 0);
    if (risk > 0) {
      totalRiskSum += risk;
      tradesWithRiskCount++;
    }
  ...
  const avgRisk = tradesWithRiskCount > 0 ? totalRiskSum / tradesWithRiskCount : 0;

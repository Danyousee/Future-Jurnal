...
import { 
  ...
  getDefaultPipValue,
  getStandardLotUnits
} from './forexCalculator';
import {
  calculateTakeProfitFromRR,
  calculateRiskAmountFromPct,
  calculateRiskPctFromAmount,
  calculateStopDistance,
  recalculateWithCustomPositionSize,
  evaluateRiskWarnings,
} from './risk';

// Backward compatibility alias routing to calculateSpotRisk with venue = 'DEX'
export const calculateDexRisk = (params: any) => calculateSpotRisk({ ...params, venue: 'DEX' });
...
export const calculatePerpetualRisk = calculateFuturesRisk;

/**
 * Universal Multi-Market Trade Risk Calculator.
 * Routes to Spot, Perpetual, or Forex calculation engine based on active tradingMode.
 */
export function calculateTradeRisk(
  state: CalculatorState,
  plan?: TradingPlan
): CalculationResult {
  const mode = state.tradingMode || 'PERPETUAL';

  // 1. SPOT TRADING (Shared calculation engine for both CEX and DEX venues)
  if (mode === 'SPOT' || (mode as any) === 'DEX') {
    const isDex = state.spotVenue === 'DEX' || (mode as any) === 'DEX';
    ...
    return calculateSpotRisk({
      ...
      venue: isDex ? 'DEX' : 'CEX',
      pair: state.pair || (isDex ? (state.dexToken ? `${state.dexToken}/WETH` : 'PEPE/WETH') : 'BTC/USDT'),
      ...
      slippagePct: isDex ? (state.dexSlippagePct ?? 0.50) : state.slippagePct,
      chain: state.dexChain || 'Ethereum',
      dex: state.dexProtocol || 'Uniswap',
      token: state.dexToken || (state.pair ? state.pair.split('/')[0] : 'PEPE'),
      tokenLiquidity: state.tokenLiquidity,
      gasFee: state.dexGasFee !== undefined ? state.dexGasFee : 8.50,
      networkFee: state.dexNetworkFee !== undefined ? state.dexNetworkFee : 0,
      dexFeePct: state.dexTradingFeePct !== undefined ? state.dexTradingFeePct : (state.dexFeePct !== undefined ? state.dexFeePct : 0.30),
      priceImpactPct: state.dexPriceImpactPct !== undefined ? state.dexPriceImpactPct : 0.15,
    });
  }
  ...
  // 3. PERPETUAL FUTURES (Default)
  return calculateFuturesRisk(state, plan);
}
...
export function validatePreTradeRiskGate(params: {
  ...
  // Max simultaneous portfolio risk budget (default 6% of capital or weekly loss allowance)
  const maxPortfolioRiskPct = plan.maxWeeklyLossPercent || 6.0;
  const maxPortfolioRiskBudget = plan.maxWeeklyLossAmount || ((startingCap * maxPortfolioRiskPct) / 100);

  // Active Open Portfolio Risk Analysis
  const currentOpenRisk = openPositions.reduce((acc, p) => acc + (p.riskAmount || 0), 0);
  const prospectiveTotalRisk = currentOpenRisk + result.riskAmount;
  const remainingPortfolioRiskCapacity = Math.max(0, maxPortfolioRiskBudget - currentOpenRisk);

  // Active Open Margin Analysis
  const currentOpenMargin = openPositions.reduce((acc, p) => acc + (p.margin || 0), 0);
  const tradeMarginReq = result.marginRequired || result.positionSize;
  const prospectiveTotalMargin = currentOpenMargin + tradeMarginReq;
  const prospectiveMarginUtilizationPct = state.accountBalance > 0 
    ? (prospectiveTotalMargin / state.accountBalance) * 100 
    : 0;

  // Correlated Group Analysis
  const prospectiveGroup = (
    state.tradingMode === 'FOREX' 
      ? 'Forex Majors' 
      : state.pair.startsWith('BTC') 
      ? 'BTC-Related' 
      : state.pair.startsWith('ETH') 
      ? 'Layer-1 Infrastructure' 
      : state.spotVenue === 'DEX' 
      ? 'Memes & DeFi' 
      : 'Altcoins'
  );
  const sameGroupPositions = openPositions.filter(
    (p) => (p.correlationGroup || '').toLowerCase() === prospectiveGroup.toLowerCase() ||
           p.pair.toUpperCase() === state.pair.toUpperCase()
  );
  const existingGroupRisk = sameGroupPositions.reduce((acc, p) => acc + (p.riskAmount || 0), 0);
  const prospectiveGroupRisk = existingGroupRisk + result.riskAmount;
  ...
  // 3. Active Portfolio Risk Capacity Check
  const portfolioRiskPass = prospectiveTotalRisk <= maxPortfolioRiskBudget || openPositions.length === 0;
  checklist.push({
    id: 'portfolio-risk-cap',
    title: 'Portfolio risk capacity',
    passed: portfolioRiskPass,
    detail: portfolioRiskPass 
      ? `Prospective open risk: $${formatNumber(prospectiveTotalRisk)} / $${formatNumber(maxPortfolioRiskBudget)} (${openPositions.length} active positions)`
      : `Prospective risk $${formatNumber(prospectiveTotalRisk)} exceeds portfolio limit of $${formatNumber(maxPortfolioRiskBudget)}`,
    fieldKey: 'riskPct',
    suggestedFix: portfolioRiskPass ? undefined : `Reduce risk % to stay under $${formatNumber(remainingPortfolioRiskCapacity)} remaining capacity`,
  });
  if (!portfolioRiskPass) {
    blockedReasons.push({
      id: 'portfolio-risk-exceeded',
      reason: `Prospective portfolio stop-risk ($${formatNumber(prospectiveTotalRisk)} / ${((prospectiveTotalRisk / startingCap) * 100).toFixed(1)}%) exceeds your portfolio risk capacity ($${formatNumber(maxPortfolioRiskBudget)} / ${maxPortfolioRiskPct}%).`,
      fieldKey: 'riskPct',
      actionText: `Lower risk % or close an existing open position. You have $${formatNumber(remainingPortfolioRiskCapacity)} in remaining risk capacity.`,
      suggestedFix: String(Math.max(0.25, Number(((remainingPortfolioRiskCapacity / startingCap) * 100).toFixed(2)))),
    });
  }

  // 4. Combined Margin Utilization Check
  if (prospectiveTotalMargin > state.accountBalance && state.accountBalance > 0) {
    blockedReasons.push({
      id: 'portfolio-margin-exceeded',
      reason: `Combined margin required ($${formatNumber(prospectiveTotalMargin)}) across ${openPositions.length + 1} positions exceeds total account equity ($${formatNumber(state.accountBalance)}).`,
      fieldKey: 'balance',
      actionText: 'De-leverage open positions or decrease position size to avoid over-collateralization.',
    });
  }

  // 5. Correlated Group Exposure Check
  if (sameGroupPositions.length >= 1 && prospectiveGroupRisk > (startingCap * 0.04)) {
    warnings.push(
      `Correlated Risk Alert: Adding this trade increases "${prospectiveGroup}" risk to $${formatNumber(prospectiveGroupRisk)} (${((prospectiveGroupRisk / startingCap) * 100).toFixed(1)}% of account) across ${sameGroupPositions.length + 1} concurrent positions.`
    );
  }

  // Multi-Market Mode awareness
  const mode = state.tradingMode || 'PERPETUAL';

  // 3. Leverage limit check
  if (mode === 'SPOT') {
    const isDexVenue = state.spotVenue === 'DEX';
    checklist.push({
      id: 'leverage',
      title: isDexVenue ? 'Spot DEX Swap (No leverage)' : 'Spot CEX execution (No leverage)',
      passed: true,
      detail: isDexVenue ? 'Direct wallet swap (Zero liquidation risk)' : '1x cash-settled position (Zero liquidation risk)',
      fieldKey: 'leverage',
    });
    // Spot Capital Limit Check
    const spotCapRequired = result.positionSize || result.marginRequired;
    const hasEnoughSpotCapital = spotCapRequired <= state.accountBalance;
    if (!hasEnoughSpotCapital && state.accountBalance > 0) {
      blockedReasons.push({
        id: isDexVenue ? 'dex-capital-exceeded' : 'spot-capital-exceeded',
        reason: isDexVenue
          ? `Swap position value ($${formatNumber(spotCapRequired)}) exceeds wallet balance ($${formatNumber(state.accountBalance)}).`
          : `Capital required ($${formatNumber(spotCapRequired)}) exceeds account balance ($${formatNumber(state.accountBalance)}).`,
        fieldKey: 'balance',
        actionText: isDexVenue
          ? 'Reduce risk percentage or allocate more wallet capital.'
          : 'Increase account balance or reduce trade risk percentage.',
      });
    }
  } else if (mode === 'FOREX') {
    const fxLev = state.forexLeverage || state.leverage || 100;
    const fxLevPass = fxLev <= 500 && fxLev >= 1;
    checklist.push({
      id: 'leverage',
      title: 'Forex Leverage',
      passed: fxLevPass,
      detail: `${fxLev}:1 broker leverage`,
      fieldKey: 'leverage',
    });
    if (!fxLevPass) {
      blockedReasons.push({
        id: 'leverage-exceeded',
        reason: `Leverage is ${fxLev}:1, maximum allowed is 500:1.`,
        fieldKey: 'leverage',
        actionText: 'Select standard broker leverage (30:1 to 500:1).',
      });
    }
    // Forex Margin Requirement check
    if (result.marginRequired > state.accountBalance && state.accountBalance > 0) {
      blockedReasons.push({
        id: 'fx-margin-exceeded',
        reason: `Margin required ($${formatNumber(result.marginRequired)}) exceeds available account equity ($${formatNumber(state.accountBalance)}).`,
        fieldKey: 'balance',
        actionText: 'Reduce risk percentage or increase leverage to lower required margin.',
      });
    }
  } else {
    // Perpetual leverage check
  ...
  const isBuyOrLong = mode === 'SPOT'
    ? (state.spotDirection === 'BUY' || state.direction === 'LONG')
  ...
  if (state.entryPrice > 0 && state.stopLoss > 0 && !directionLogicPass) {
    blockedReasons.push({
      ...
      reason: isBuyOrLong
        ? `Stop-Loss ($${formatNumber(state.stopLoss)}) must be below Entry Price ($${formatNumber(state.entryPrice)}) for BUY/LONG.`
        : `Stop-Loss ($${formatNumber(state.stopLoss)}) must be above Entry Price ($${formatNumber(state.entryPrice)}) for SELL/SHORT.`,
      fieldKey: 'stopLoss',
      actionText: isBuyOrLong
  ...
  // 10. Liquidation safety buffer check (Perpetuals only)
  if (mode === 'PERPETUAL') {
  ...
  // 11. Preferred crypto check (Crypto Spot & Perpetuals only)
  if (mode !== 'FOREX' && state.pair && plan.preferredCryptos && plan.preferredCryptos.length > 0) {
  ...
  return {
    ...
    metrics: {
      ...
      currentOpenRisk,
      prospectiveTotalRisk,
      maxPortfolioRiskBudget,
      remainingPortfolioRiskCapacity,
      prospectiveMarginUtilizationPct,
      correlatedGroupRisk: prospectiveGroupRisk,
      correlatedGroupName: prospectiveGroup,
    },
  };
}

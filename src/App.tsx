...
import { 
  calculateWeeklyPerformanceSummary, 
  getIsMondaySimulationActive, 
  setMondaySimulationActive 
} from './utils/weeklyNotificationTrigger';
import { 
  db, 
  initializeDatabase, 
  loadTradingPlan, 
...
export default function App() {
  ...
  const refreshTrades = useCallback(async () => {
    try {
      ...
      const allTrades = await loadJournalTrades();
      setTrades(allTrades);
  ...
  // Monday Morning Weekly Performance Trigger
  const weeklySummary = useMemo(() => {
    return calculateWeeklyPerformanceSummary(trades, openPositions, {
      startingCapital: stats.startingBalance || plan.startingCapital || 10000,
    });
  }, [trades, openPositions, stats.startingBalance, plan.startingCapital]);
  ...
  const handleSaveToJournalFromCalculator = (
    ...
    const mode = calcState.tradingMode || 'PERPETUAL';
    const effectiveLev = mode === 'SPOT' ? 1 : mode === 'FOREX' ? (calcState.forexLeverage || 100) : calcState.leverage;
    const isDexSpot = mode === 'SPOT' && calcState.spotVenue === 'DEX';

    setCalculatorDraft({
      tradeMode: mode,
      spotVenue: mode === 'SPOT' ? (calcState.spotVenue || 'CEX') : undefined,
      pair: calcState.pair,
      ...
      tradingFee: isDexSpot ? (calcState.dexTradingFeePct ? calcState.dexTradingFeePct / 100 : 0.003) : calcState.feeRate,
      lotSize: result.forex?.lotSize,
      pipsRisk: result.forex?.stopLossPips,
      pipsGain: result.forex?.takeProfitPips,
      capitalRequired: result.marginRequired || result.positionSize,
      dexChain: isDexSpot ? (calcState.dexChain || 'Ethereum') : undefined,
      dexProtocol: isDexSpot ? (calcState.dexProtocol || 'Uniswap') : undefined,
      dexToken: isDexSpot ? (calcState.dexToken || calcState.pair) : undefined,
      dexWallet: isDexSpot ? (calcState.dexWallet || 'MetaMask') : undefined,
      dexGasFee: isDexSpot ? calcState.dexGasFee : undefined,
      dexTradingFeePct: isDexSpot ? calcState.dexTradingFeePct : undefined,
      dexPriceImpactPct: isDexSpot ? calcState.dexPriceImpactPct : undefined,
      dexSlippagePct: isDexSpot ? calcState.dexSlippagePct : undefined,
      tokenLiquidity: isDexSpot ? calcState.tokenLiquidity : undefined,
      date: new Date().toISOString().split('T')[0],
      time: new Date().toTimeString().slice(0, 5),
      strategy: isDexSpot ? 'Spot DEX On-Chain Swap' : mode === 'SPOT' ? 'Spot CEX Cash Setup' : mode === 'FOREX' ? 'Forex Currency Setup' : 'Perpetual Futures Setup',
      approvalRecord: approvalRecord,
      notes: `Captured from ${mode}${mode === 'SPOT' ? ` (${calcState.spotVenue || 'CEX'})` : ''} Calculator. Risk target: ${calcState.riskPercentage}% ($${result.riskAmount.toFixed(2)}). ${mode === 'FOREX' ? `Lots: ${result.forex?.lotSize || 0}, SL: ${result.forex?.stopLossPips} pips` : isDexSpot ? `DEX Swap: ${result.quantity?.toFixed(4)} ${calcState.dexToken || calcState.pair} on ${calcState.dexChain || 'Ethereum'} (Gas: $${calcState.dexGasFee || 0})` : mode === 'SPOT' ? `Spot Cash Sized: ${result.quantity?.toFixed(4)} coins` : `Est. Liq: $${result.liquidationPrice.toFixed(2)}`}.${approvalRecord ? ` Gate Status: ${approvalRecord.status} (Score: ${approvalRecord.score}%)` : ''}`,
    });
    setEditingTrade(null);
    setIsTradeModalOpen(true);
  };
  ...
  return (
    <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-slate-50 text-slate-900 font-sans transition-colors flex flex-col antialiased selection:bg-[#1565ff] selection:text-white">
      ...
      <Navbar
        ...
        hasWeeklyTriggerActive={weeklySummary.isTriggered}
      />
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 pt-4 pb-36 sm:pb-32 overflow-x-hidden">
        ...
        {activeTab === 'calculator' && (
          <RiskCalculator 
            ...
            onOpenPosition={handleSaveOpenPosition}
      ...
      {/* Floating In-App Price Alert Toast Notification */}
      <PriceAlertToast onNavigateToCalculator={() => setActiveTab('calculator')} />
      ...
      <NotificationModal
        ...
        onToggleMondaySimulation={() => {
          setMondaySimulationActive(!weeklySummary.isSimulated);
          refreshTrades();
        }}
      />

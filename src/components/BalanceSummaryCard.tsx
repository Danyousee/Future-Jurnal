...
export const BalanceSummaryCard: React.FC<BalanceSummaryCardProps> = ({
  ...
  return (
    <div 
      ...
      className="w-full max-w-md sm:max-w-lg mx-auto bg-white rounded-xl sm:rounded-2xl px-3.5 py-2 sm:px-4 sm:py-2.5 shadow-md shadow-blue-900/10 border border-white/70 space-y-1.5 transition-all"
    >
      {/* 💼 Total Balance: Label with Wallet Icon & Formatted Balance */}
      <div className="flex items-center gap-2 pt-0">
        <div className="w-7 h-7 sm:w-7.5 sm:h-7.5 rounded-lg bg-blue-50/80 border border-blue-100 flex items-center justify-center text-[#1565ff] shrink-0">
          <Wallet className="w-3.5 h-3.5 stroke-[2.2]" />
        </div>
        <div className="min-w-0">
          <span 
            id="balance-summary-total-label" 
            className="text-[10px] sm:text-[11px] font-medium text-slate-500 block leading-none mb-0.5"
          >
            Total balance
          </span>
          <div className="text-base sm:text-lg font-black text-slate-900 tracking-tight font-mono truncate leading-none">

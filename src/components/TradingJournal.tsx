...
import { formatCurrency, formatNumber, formatPrice } from '../utils/calculator';
...
export const TradingJournal: React.FC<TradingJournalProps> = ({
  ...
  const [modeFilter, setModeFilter] = useState<'ALL' | 'SPOT' | 'PERPETUAL' | 'FOREX'>('ALL');
  ...
  const filteredTrades = useMemo(() => {
    return trades
      .filter((trade) => {
        ...
        // Market Mode filter
        if (modeFilter !== 'ALL') {
          const tMode = (trade.tradeMode || 'PERPETUAL').toUpperCase();
          if (modeFilter === 'SPOT') {
            const isSpotLike = tMode === 'SPOT' || tMode === 'DEX';
            if (!isSpotLike) return false;
            if (spotVenueFilter !== 'ALL') {
              const venue = (trade.spotVenue || (tMode === 'DEX' ? 'DEX' : 'CEX')).toUpperCase();
              if (venue !== spotVenueFilter) return false;
            }
          } else {
            if (tMode !== modeFilter) return false;
          }
        }
  ...
  }, [trades, searchTerm, modeFilter, directionFilter, resultFilter, strategyFilter, timeframeFilter, monthFilter, sortBy]);
            ...
            <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-2.5 sm:gap-3">
              ...
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 w-full md:w-auto justify-between md:justify-end">
                ...
                <div className="flex items-center bg-slate-100 p-0.5 sm:p-1 rounded-lg border border-slate-200 text-[11px] sm:text-xs font-semibold">
                  {(['ALL', 'SPOT', 'PERPETUAL', 'FOREX'] as const).map((m) => (
                    <button
                      key={m}
                ...
                {/* Spot Venue Sub-Filter (shown when SPOT selected) */}
                {modeFilter === 'SPOT' && (
                  <div className="flex items-center bg-emerald-50 p-0.5 sm:p-1 rounded-lg border border-emerald-200 text-[10px] sm:text-[11px] font-semibold animate-fadeIn">
                    {(['ALL', 'CEX', 'DEX'] as const).map((v) => (
                      <button
                        key={v}
                        onClick={() => setSpotVenueFilter(v)}
                        className={`px-2 py-0.5 sm:py-1 rounded transition-all cursor-pointer ${
                          spotVenueFilter === v
                  ...
                  >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                            ...
                            {(() => {
                              ...
                              const isForex = rawMode === 'FOREX';

                              if (isDex) {
                                return (
                                  <span className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded uppercase bg-purple-50 text-purple-700 border border-purple-200">
                                    SPOT • DEX{trade.dexChain ? ` • ${trade.dexChain}` : ''}
                                  </span>
                                );
                              }
                              if (isSpot) {
                                return (
                                  <span className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded uppercase bg-emerald-50 text-emerald-700 border border-emerald-200">
                                    SPOT • CEX
                                  </span>
                                );
                              }
                              if (isForex) {
                                return (
                                  <span className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded uppercase bg-amber-50 text-amber-700 border border-amber-200">
                                    FOREX{trade.lotSize ? ` ${trade.lotSize}L` : ''}
                                  </span>
                                );
                              }
                              return (
                                <span className="text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded uppercase bg-blue-50 text-blue-700 border border-blue-200">
                                  PERP
                                </span>
                              );
                            })()}
                            ...
                            >
                              {trade.direction} {(trade.tradeMode || 'PERPETUAL') === 'SPOT' || (trade.tradeMode || 'PERPETUAL') === 'DEX' ? '1x Spot' : `${trade.leverage}x`}
                            </span>
                    ...
                    {isExpanded && (
                      <div className="px-3.5 sm:px-5 py-3 sm:py-4 border-t border-slate-100 bg-slate-50/70 space-y-3 sm:space-y-4 text-xs">
                        ...
                        {/* Mode-Specific Execution Details Card */}
                        {(() => {
                          const rawMode = (trade.tradeMode || 'PERPETUAL').toUpperCase();
                          const isDex = rawMode === 'DEX' || (rawMode === 'SPOT' && String(trade.spotVenue).toUpperCase() === 'DEX');
                          const isSpot = rawMode === 'SPOT' || isDex;
                          ...
                          if (isDex) {
                            return (
                              <div className="bg-purple-50/80 p-3 sm:p-4 rounded-xl border border-purple-200/90 shadow-xs space-y-2.5">
                                <div className="flex items-center justify-between border-b border-purple-200/70 pb-2">
                                  <span className="font-bold text-purple-900 text-xs flex items-center gap-1.5">
                                    ...
                                    Spot DEX On-Chain Swap Execution
                                  </span>
                                  <span className="text-[10px] font-mono font-semibold px-2 py-0.5 rounded bg-white text-purple-700 border border-purple-200">
                                    {trade.dexChain || 'On-Chain'} • {trade.dexProtocol || 'AMM'}
                                  </span>
                                </div>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 text-xs">
                                  <div>
                                    <span className="text-purple-700 font-semibold block text-[10px] uppercase">Network / Chain</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{trade.dexChain || trade.chain || 'Ethereum'}</span>
                                  </div>
                                  <div>
                                    <span className="text-purple-700 font-semibold block text-[10px] uppercase">DEX Protocol</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{trade.dexProtocol || trade.dex || 'Uniswap'}</span>
                                  </div>
                                  <div>
                                    <span className="text-purple-700 font-semibold block text-[10px] uppercase">Slippage Tolerance</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{trade.dexSlippagePct !== undefined ? `${trade.dexSlippagePct}%` : trade.slippagePct !== undefined ? `${trade.slippagePct}%` : '0.50%'}</span>
                                  </div>
                                  <div>
                                    <span className="text-purple-700 font-semibold block text-[10px] uppercase">Price Impact</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{trade.dexPriceImpactPct !== undefined ? `${trade.dexPriceImpactPct}%` : '0.00%'}</span>
                                  </div>
                                  <div>
                                    <span className="text-purple-700 font-semibold block text-[10px] uppercase">Gas Fee Paid</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">${formatNumber(trade.dexGasFee || trade.gasFee || 0, 2)}</span>
                                  </div>
                                  <div>
                                    <span className="text-purple-700 font-semibold block text-[10px] uppercase">Network Fee</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">${formatNumber(trade.dexNetworkFee || trade.networkFee || 0, 2)}</span>
                                  </div>
                                  <div>
                                    <span className="text-purple-700 font-semibold block text-[10px] uppercase">DEX Fee %</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{trade.dexTradingFeePct || trade.dexFeePct || 0.30}%</span>
                                  </div>
                                  <div>
                                    <span className="text-purple-700 font-semibold block text-[10px] uppercase">Wallet / Custody</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs truncate block">{trade.dexWallet || trade.wallet || 'Self-Custodial'}</span>
                                  </div>
                                </div>
                              </div>
                            );
                          ...
                          if (isSpot) {
                            return (
                              <div className="bg-emerald-50/80 p-3 sm:p-4 rounded-xl border border-emerald-200/90 shadow-xs space-y-2.5">
                                <div className="flex items-center justify-between border-b border-emerald-200/70 pb-2">
                                  <span className="font-bold text-emerald-900 text-xs flex items-center gap-1.5">
                                    <span className="w-2 h-2 rounded-full bg-emerald-600"></span>
                                    Spot CEX Cash Execution
                                  </span>
                                  <span className="text-[10px] font-mono font-semibold px-2 py-0.5 rounded bg-white text-emerald-700 border border-emerald-200">
                                    Venue: CEX • 1x Cash-Settled
                                  </span>
                                </div>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 text-xs">
                                  <div>
                                    <span className="text-emerald-700 font-semibold block text-[10px] uppercase">Exchange Venue</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{trade.exchange} (Spot)</span>
                                  </div>
                                  <div>
                                    <span className="text-emerald-700 font-semibold block text-[10px] uppercase">Spot Venue Mode</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{trade.spotVenue || 'CEX'}</span>
                                  </div>
                                  <div>
                                    <span className="text-emerald-700 font-semibold block text-[10px] uppercase">Capital Required</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{formatCurrency(trade.capitalRequired || trade.positionSize)}</span>
                                  </div>
                                  <div>
                                    <span className="text-emerald-700 font-semibold block text-[10px] uppercase">Liquidation Risk</span>
                                    <span className="text-emerald-800 font-bold font-mono text-xs">0% (Unleveraged Spot)</span>
                                  </div>
                                </div>
                              </div>
                            );
                          ...
                          if (isForex) {
                            return (
                              <div className="bg-amber-50/80 p-3 sm:p-4 rounded-xl border border-amber-200/90 shadow-xs space-y-2.5">
                                <div className="flex items-center justify-between border-b border-amber-200/70 pb-2">
                                  <span className="font-bold text-amber-900 text-xs flex items-center gap-1.5">
                                    <span className="w-2 h-2 rounded-full bg-amber-600"></span>
                                    Forex Currency Pair Execution
                                  </span>
                                  <span className="text-[10px] font-mono font-semibold px-2 py-0.5 rounded bg-white text-amber-800 border border-amber-200">
                                    {trade.pair} • Lot Sized
                                  </span>
                                </div>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 text-xs">
                                  <div>
                                    <span className="text-amber-700 font-semibold block text-[10px] uppercase">Currency Pair</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{trade.pair}</span>
                                  </div>
                                  <div>
                                    <span className="text-amber-700 font-semibold block text-[10px] uppercase">Lot Size</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{trade.lotSize || 0.10} Lots</span>
                                  </div>
                                  <div>
                                    <span className="text-amber-700 font-semibold block text-[10px] uppercase">Pip Risk (SL Distance)</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{trade.pipsRisk !== undefined ? `${trade.pipsRisk} pips` : 'N/A'}</span>
                                  </div>
                                  <div>
                                    <span className="text-amber-700 font-semibold block text-[10px] uppercase">Pip Gain Target</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{trade.pipsGain !== undefined ? `${trade.pipsGain} pips` : 'N/A'}</span>
                                  </div>
                                  <div>
                                    <span className="text-amber-700 font-semibold block text-[10px] uppercase">Typical Spread</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">1.2 pips</span>
                                  </div>
                                  <div>
                                    <span className="text-amber-700 font-semibold block text-[10px] uppercase">Broker Commission</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{formatCurrency((trade.lotSize || 0.1) * 7)}</span>
                                  </div>
                                  <div>
                                    <span className="text-amber-700 font-semibold block text-[10px] uppercase">Forex Leverage</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{trade.leverage || 100}x</span>
                                  </div>
                                  <div>
                                    <span className="text-amber-700 font-semibold block text-[10px] uppercase">Margin Required</span>
                                    <span className="text-slate-900 font-bold font-mono text-xs">{formatCurrency(trade.margin || (trade.positionSize / (trade.leverage || 100)))}</span>
                                  </div>
                                </div>
                              </div>
                            );
                          ...
                          // Perpetual Futures
                          return (
                            <div className="bg-blue-50/80 p-3 sm:p-4 rounded-xl border border-blue-200/90 shadow-xs space-y-2.5">
                              <div className="flex items-center justify-between border-b border-blue-200/70 pb-2">
                                <span className="font-bold text-blue-900 text-xs flex items-center gap-1.5">
                                  <span className="w-2 h-2 rounded-full bg-blue-600"></span>
                                  Perpetual Futures Contract Execution
                                </span>
                                <span className="text-[10px] font-mono font-semibold px-2 py-0.5 rounded bg-white text-blue-800 border border-blue-200">
                                  {trade.leverage || 10}x Leverage • Margin-Backed
                                </span>
                              </div>
                              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 text-xs">
                                <div>
                                  <span className="text-blue-700 font-semibold block text-[10px] uppercase">Leverage</span>
                                  <span className="text-slate-900 font-bold font-mono text-xs">{trade.leverage || 10}x</span>
                                </div>
                                <div>
                                  <span className="text-blue-700 font-semibold block text-[10px] uppercase">Margin Required</span>
                                  <span className="text-slate-900 font-bold font-mono text-xs">{formatCurrency(trade.margin || (trade.positionSize / (trade.leverage || 10)))}</span>
                                </div>
                                <div>
                                  <span className="text-blue-700 font-semibold block text-[10px] uppercase">Net Funding</span>
                                  <span className="text-slate-900 font-bold font-mono text-xs">{trade.fundingCost !== undefined ? (trade.fundingCost >= 0 ? `+${formatCurrency(trade.fundingCost)}` : formatCurrency(trade.fundingCost)) : '0.01% / 8h'}</span>
                                </div>
                                <div>
                                  <span className="text-blue-700 font-semibold block text-[10px] uppercase">Fee Tier / Rate</span>
                                  <span className="text-slate-900 font-bold font-mono text-xs">{trade.tradingFee || 0.055}%</span>
                                </div>
                              </div>
                            </div>
                          );
                        ...
                        {/* Planned vs Actual Analysis Card */}
                        {(trade.stopLoss > 0 || trade.takeProfit > 0 || trade.exitPrice > 0) && (() => {
                          const plannedRiskDist = Math.abs(trade.entryPrice - trade.stopLoss);
                          const plannedRewardDist = trade.takeProfit > 0 ? Math.abs(trade.takeProfit - trade.entryPrice) : 0;
                          const targetRR = plannedRiskDist > 0 && plannedRewardDist > 0 ? (plannedRewardDist / plannedRiskDist) : 0;
                          const actualRR = trade.rMultiple !== undefined ? trade.rMultiple : (trade.riskAmount > 0 ? trade.pnl / trade.riskAmount : 0);

                          return (
                            <div className="bg-white p-3 sm:p-3.5 rounded-lg border border-slate-200 shadow-xs space-y-2">
                              <div className="flex items-center justify-between">
                                <span className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                                  <Scale className="w-4 h-4 text-[#1565ff]" />
                                  Planned vs. Actual Execution Variance
                                </span>
                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                                  actualRR >= targetRR && targetRR > 0
                                    ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
                                    : 'bg-slate-50 text-slate-700 border-slate-200'
                                }`}>
                                  {actualRR >= 0 ? '+' : ''}{actualRR.toFixed(2)}R Realized
                                </span>
                              </div>

                              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                                <div className="bg-slate-50 p-2 rounded border border-slate-100">
                                  <span className="text-[10px] uppercase font-bold text-slate-400 block">Planned Target R:R</span>
                                  <div className="font-mono font-bold text-slate-800 mt-0.5">
                                    {targetRR > 0 ? `1:${targetRR.toFixed(2)}` : 'N/A'}
                                  </div>
                                </div>

                                <div className="bg-slate-50 p-2 rounded border border-slate-100">
                                  <span className="text-[10px] uppercase font-bold text-slate-400 block">Actual Realized R</span>
                                  <div className={`font-mono font-bold mt-0.5 ${actualRR >= 0 ? 'text-[#22a65e]' : 'text-[#ff3b4a]'}`}>
                                    {actualRR >= 0 ? '+' : ''}{actualRR.toFixed(2)}R
                                  </div>
                                </div>

                                <div className="bg-slate-50 p-2 rounded border border-slate-100">
                                  <span className="text-[10px] uppercase font-bold text-slate-400 block">Planned Target TP</span>
                                  <div className="font-mono font-bold text-[#22a65e] mt-0.5 truncate">
                                    {trade.takeProfit > 0 ? formatPrice(trade.takeProfit) : 'N/A'}
                                  </div>
                                </div>

                                <div className="bg-slate-50 p-2 rounded border border-slate-100">
                                  <span className="text-[10px] uppercase font-bold text-slate-400 block">Actual Exit</span>
                                  <div className="font-mono font-bold text-slate-900 mt-0.5 truncate">
                                    {trade.exitPrice > 0 ? formatPrice(trade.exitPrice) : 'Open'}
                                  </div>
                                </div>
                              </div>
                            </div>

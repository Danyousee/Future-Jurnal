...
export const AnalyticsView: React.FC<AnalyticsViewProps> = ({
  ...
  // Monthly trajectory controls
  const [monthlyChartRange, setMonthlyChartRange] = useState<'12m' | 'all'>('12m');
  const [monthlyViewMode, setMonthlyViewMode] = useState<'both' | 'chart' | 'breakdown'>('both');
  ...
  // Trailing 12 months P&L trajectory calculation
  const trailing12MonthsData = useMemo(() => {
    let refDate = new Date();
    if (filteredTrades.length > 0) {
      const validDates = filteredTrades
        .map((t) => new Date(t.date).getTime())
        .filter((time) => !isNaN(time));
      if (validDates.length > 0) {
        const latestTime = Math.max(...validDates);
        if (latestTime > refDate.getTime()) {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      const displayLabel = `${shortMonthNames[m]} '${String(y).slice(2)}`;
      const label = `${fullMonthNames[m]} ${y}`;
      monthSlots.push({ monthKey, label, displayLabel, year: y, month: m });
    }

    const statMap = new Map<string, MonthlyStat>();
    activeMonthlyStats.forEach((st) => {
      statMap.set(st.monthKey, st);
    });

    let total12mPnl = 0;
    let total12mTrades = 0;
    let total12mWins = 0;
    let total12mLosses = 0;
    let activeMonthsCount = 0;
    let profitableMonthsCount = 0;

    const chartData = monthSlots.map((slot) => {
      const stat = statMap.get(slot.monthKey);
      const pnl = stat ? stat.pnl : 0;
      const tradesCount = stat ? stat.trades : 0;
      const wins = stat ? stat.wins : 0;
      const losses = stat ? stat.losses : 0;
      const winRate = tradesCount > 0 ? (wins / tradesCount) * 100 : 0;

      if (tradesCount > 0 || pnl !== 0) {
        total12mTrades += tradesCount;
        total12mWins += wins;
        total12mLosses += losses;
        activeMonthsCount++;
        if (pnl > 0) profitableMonthsCount++;
      }
      total12mPnl += pnl;
      ...
      return {
        ...
        isPositive: pnl >= 0,
        hasActivity: tradesCount > 0 || pnl !== 0,
      };
    });

    const activeMonthsOnly = chartData.filter((d) => d.hasActivity);
    const bestMonth = activeMonthsOnly.length > 0 
      ? activeMonthsOnly.reduce((best, cur) => cur.pnl > best.pnl ? cur : best, activeMonthsOnly[0])
      : null;
    const worstMonth = activeMonthsOnly.length > 0 
      ? activeMonthsOnly.reduce((worst, cur) => cur.pnl < worst.pnl ? cur : worst, activeMonthsOnly[0])
      : null;

    const avgMonthlyPnl = activeMonthsCount > 0 ? total12mPnl / activeMonthsCount : 0;
    const overallWinRate = total12mTrades > 0 ? (total12mWins / total12mTrades) * 100 : 0;

    const allMonthsData = activeMonthlyStats.map((st) => {
      const parts = st.monthKey.split('-');
      const y = parts[0];
      const mIdx = parseInt(parts[1], 10) - 1;
      const displayLabel = mIdx >= 0 && mIdx < 12 ? `${shortMonthNames[mIdx]} '${y.slice(2)}` : st.label;
      return {
  ...
  }, [activeMonthlyStats, filteredTrades]);
  ...
  const renderWidget = (widget: AnalyticsWidgetConfig) => {
    switch (widget.id) {
      ...
      // 4. MONTHLY P&L DISTRIBUTION & 12-MONTH TRAJECTORY
      case 'monthly_pnl': {
        const displayedChartData = monthlyChartRange === '12m' 
          ? trailing12MonthsData.chartData 
          : (trailing12MonthsData.allMonthsData.length > 0 ? trailing12MonthsData.allMonthsData : trailing12MonthsData.chartData);

        const hasTrades = trailing12MonthsData.total12mTrades > 0 || activeMonthlyStats.length > 0;
        const isOverallPositive = trailing12MonthsData.total12mPnl >= 0;

        return (
          <div key="monthly_pnl" className="bg-white p-3.5 sm:p-5 lg:p-6 rounded-xl border border-slate-200/90 shadow-md space-y-4">
            {/* Header & Controls */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-100/70 text-[#1565ff]">
                    {monthlyChartRange === '12m' ? 'Last 12 Months' : `${activeMonthlyStats.length} Months Tracked`}
                  </span>
                </div>
                <p className="text-[11px] text-slate-500 mt-0.5">
                  Monthly Net P&L distribution and performance trajectory across your trading history
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="inline-flex rounded-lg bg-slate-100 p-0.5 border border-slate-200 text-[11px] font-semibold">
                  <button
                    type="button"
                    onClick={() => setMonthlyChartRange('12m')}
                    className={`px-2.5 py-1 rounded-md transition-all cursor-pointer ${
                      monthlyChartRange === '12m'
                  ...
                  <button
                    type="button"
                    onClick={() => setMonthlyChartRange('all')}
                    className={`px-2.5 py-1 rounded-md transition-all cursor-pointer ${
                      monthlyChartRange === 'all'
                        ? 'bg-white text-slate-900 shadow-xs'
                        : 'text-slate-500 hover:text-slate-800'
                    }`}
                  >
                    All ({activeMonthlyStats.length})
                  </button>
                </div>
                ...
                <div className="inline-flex rounded-lg bg-slate-100 p-0.5 border border-slate-200 text-[11px] font-semibold">
                  <button
                    type="button"
                    onClick={() => setMonthlyViewMode('both')}
                    className={`px-2 py-1 rounded-md transition-all cursor-pointer ${
                      monthlyViewMode === 'both'
                        ? 'bg-white text-slate-900 shadow-xs'
                        : 'text-slate-500 hover:text-slate-800'
                    }`}
                    title="Show chart and breakdown"
                  >
                    Both
                  </button>
                  <button
                  ...
                  <button
                    type="button"
                    onClick={() => setMonthlyViewMode('breakdown')}
                    className={`px-2 py-1 rounded-md transition-all cursor-pointer ${
                      monthlyViewMode === 'breakdown'
                        ? 'bg-white text-slate-900 shadow-xs'
                        : 'text-slate-500 hover:text-slate-800'
                    }`}
                    title="Show table breakdown only"
                  >
                    List
                  </button>
                </div>
            ...
            {!hasTrades ? (
              <div className="p-8 sm:p-12 text-center rounded-xl bg-slate-50 border border-dashed border-slate-200 space-y-2">
                <Calendar className="w-8 h-8 text-slate-300 mx-auto" />
                <h4 className="text-xs font-bold text-slate-700">No Monthly Trade Data Available</h4>
                <p className="text-[11px] text-slate-500 max-w-sm mx-auto">
                  Log closed trades in the Trading Journal to generate your monthly PnL trajectory bar chart and performance statistics.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {/* Trajectory Highlight Cards */}
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3">
                  {/* 12M / Filtered Net P&L */}
                  <div className="p-3 rounded-xl bg-slate-50/80 border border-slate-200/80">
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">
                      {monthlyChartRange === '12m' ? '12-Month Net P&L' : 'All Months Net P&L'}
                    </span>
                    <div className="flex items-baseline gap-1.5 mt-1">
                      <span className={`text-sm sm:text-base font-extrabold font-mono ${isOverallPositive ? 'text-[#22a65e]' : 'text-[#ff3b4a]'}`}>
                        {isOverallPositive ? '+' : ''}{formatCurrency(trailing12MonthsData.total12mPnl)}
                      </span>
                    </div>
                    <span className="text-[10px] text-slate-400 font-mono mt-0.5 block">
                      {trailing12MonthsData.total12mTrades} trades • {trailing12MonthsData.overallWinRate.toFixed(1)}% Win Rate
                    </span>
                  </div>
                  ...
                  <div className="p-3 rounded-xl bg-slate-50/80 border border-slate-200/80">
                    ...
                    <div className="flex items-baseline gap-1.5 mt-1">
                      <span className="text-sm sm:text-base font-extrabold font-mono text-slate-900">
                        {trailing12MonthsData.profitableMonthsCount} / {displayedChartData.length}
                      </span>
                      <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-blue-100 text-[#1565ff]">
                        {displayedChartData.length > 0 ? `${Math.round((trailing12MonthsData.profitableMonthsCount / displayedChartData.length) * 100)}%` : '0%'}
                      </span>
                    </div>
                    <span className="text-[10px] text-slate-400 font-mono mt-0.5 block">
                      Monthly win consistency
                    </span>
                  </div>
                  ...
                  <div className="p-3 rounded-xl bg-slate-50/80 border border-slate-200/80">
                    ...
                    <div className="flex items-baseline gap-1.5 mt-1">
                      <span className={`text-sm sm:text-base font-extrabold font-mono ${trailing12MonthsData.avgMonthlyPnl >= 0 ? 'text-[#22a65e]' : 'text-[#ff3b4a]'}`}>
                        {trailing12MonthsData.avgMonthlyPnl >= 0 ? '+' : ''}{formatCurrency(trailing12MonthsData.avgMonthlyPnl)}
                      </span>
                    </div>
                    <span className="text-[10px] text-slate-400 font-mono mt-0.5 block">
                      {trailing12MonthsData.activeMonthsCount} active trading {trailing12MonthsData.activeMonthsCount === 1 ? 'month' : 'months'}
                    </span>
                  </div>
                  ...
                  <div className="p-3 rounded-xl bg-slate-50/80 border border-slate-200/80">
                    ...
                    <div className="flex items-baseline gap-1.5 mt-1">
                      {trailing12MonthsData.bestMonth ? (
                        <>
                          <span className={`text-sm sm:text-base font-extrabold font-mono ${trailing12MonthsData.bestMonth.pnl >= 0 ? 'text-[#22a65e]' : 'text-slate-700'}`}>
                            {trailing12MonthsData.bestMonth.pnl >= 0 ? '+' : ''}{formatCurrency(trailing12MonthsData.bestMonth.pnl)}
                          </span>
                        </>
                      ) : (
                        <span className="text-sm font-bold text-slate-400 font-mono">$0.00</span>
                      )}
                    </div>
                    <span className="text-[10px] text-slate-400 font-mono mt-0.5 block truncate">
                      {trailing12MonthsData.bestMonth ? `${trailing12MonthsData.bestMonth.label} (${trailing12MonthsData.bestMonth.trades} trades)` : 'No active trades'}
                    </span>
                  </div>
                </div>

                {/* Recharts Monthly P&L Bar Chart */}
                {(monthlyViewMode === 'both' || monthlyViewMode === 'chart') && (
                  <div className="p-3 sm:p-4 rounded-xl bg-slate-50/50 border border-slate-200/90 space-y-2">
                    <div className="flex items-center justify-between text-[11px] font-bold text-slate-600 px-1">
                      <span className="flex items-center gap-1.5">
                        <BarChart3 className="w-3.5 h-3.5 text-[#1565ff]" />
                        Net Profit & Loss Trajectory ($)
                      </span>
                      <div className="flex items-center gap-3 text-[10px] font-medium text-slate-500">
                        <span className="flex items-center gap-1">
                          <span className="w-2.5 h-2.5 rounded-xs bg-[#22a65e]"></span> Profit Month
                        </span>
                        <span className="flex items-center gap-1">
                          <span className="w-2.5 h-2.5 rounded-xs bg-[#ff3b4a]"></span> Loss Month
                        </span>
                        <span className="flex items-center gap-1">
                          <span className="w-2.5 h-2.5 rounded-xs bg-slate-300"></span> Inactive / Flat
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="w-full h-64 sm:h-72 pt-2">
                      <ResponsiveContainer width="100%" height="100%">
                        ...
                        >
                          ...
                          <RechartsTooltip
                            ...
                            content={({ active, payload }) => {
                              if (active && payload && payload.length) {
                                ...
                                return (
                                  <div className="bg-slate-900/95 backdrop-blur-md text-white p-3 rounded-xl text-xs shadow-xl border border-slate-800 space-y-1.5 min-w-[190px]">
                                    <div className="font-bold text-slate-100 border-b border-slate-800/80 pb-1.5 flex items-center justify-between">
                                      ...
                                      >
                                        {data.pnl > 0 ? 'PROFIT' : data.pnl < 0 ? 'LOSS' : 'BREAKEVEN'}
                                      </span>
                                    </div>
                                    <div className="flex items-center justify-between pt-0.5 font-mono">
                                      <span className="text-slate-400 text-[11px]">Net P&L:</span>
                                      <span className={`font-bold ${data.pnl >= 0 ? 'text-[#22a65e]' : 'text-[#ff3b4a]'}`}>
                                        {data.pnl >= 0 ? '+' : ''}{formatCurrency(data.pnl)}
                                      </span>
                                    </div>
                                    <div className="flex items-center justify-between text-[11px] font-mono">
                                      <span className="text-slate-400">Total Trades:</span>
                                      <span className="text-slate-200">{data.trades} ({data.wins}W / {data.losses}L)</span>
                                    </div>
                                    <div className="flex items-center justify-between text-[11px] font-mono">
                                      <span className="text-slate-400">Win Rate:</span>
                                      <span className="text-slate-200">{data.trades > 0 ? `${data.winRate.toFixed(1)}%` : 'N/A'}</span>
                                    </div>
                                  </div>
                                );
                              }
                          ...
                          <Bar dataKey="pnl" maxBarSize={38} radius={[3, 3, 0, 0]}>
                            {displayedChartData.map((entry, index) => (
                              <Cell
                                key={`cell-${entry.monthKey || index}`}
                                fill={entry.pnl > 0 ? '#22a65e' : entry.pnl < 0 ? '#ff3b4a' : '#cbd5e1'}
                              />
                            ))}
                          </Bar>
                        </BarChart>
                ...
                {/* Monthly Detailed Breakdown List */}
                {(monthlyViewMode === 'both' || monthlyViewMode === 'breakdown') && (
                  <div className="space-y-2 pt-1">
                    <div className="flex items-center justify-between text-[11px] font-bold text-slate-600 px-0.5">
                      <span>Monthly Performance Breakdown ({displayedChartData.filter(d => d.hasActivity).length} active months)</span>
                      <span className="text-slate-400 font-mono text-[10px]">Net P&L • Progress</span>
                    </div>

                    <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                      {displayedChartData
                        .filter(d => monthlyChartRange === 'all' ? true : d.hasActivity || displayedChartData.length <= 12)
                        .map((m) => {
                          const isProfitable = m.pnl >= 0;
                          const maxMonthlyAbs = Math.max(
                            ...displayedChartData.map((item) => Math.abs(item.pnl)),
                            100
                          );
                          const barWidth = m.pnl === 0 ? 0 : Math.min(100, Math.max(8, (Math.abs(m.pnl) / maxMonthlyAbs) * 100));
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  {m.trades > 0 && (
                                    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                                      m.winRate >= 50 ? 'bg-emerald-50 text-[#22a65e]' : 'bg-rose-50 text-[#ff3b4a]'
                                    }`}>
                                      {m.winRate.toFixed(0)}% WR
                                    </span>
                                  )}
                                </div>
                                <span className={`font-bold ${m.pnl === 0 ? 'text-slate-400' : isProfitable ? 'text-[#22a65e]' : 'text-[#ff3b4a]'}`}>
                                  {m.pnl > 0 ? '+' : ''}{formatCurrency(m.pnl)}
                                </span>
                              </div>

                              <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden flex items-center p-0 mt-2">
                                <div
                                  className={`h-full rounded-full transition-all ${
                                    m.pnl === 0 ? 'bg-transparent' : isProfitable ? 'bg-[#22a65e]' : 'bg-[#ff3b4a]'
                                  }`}

...
interface NavbarProps {
  ...
  pageTitle?: string;
  hasWeeklyTriggerActive?: boolean;
}
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    ...
    >
      {/* Top Header: Hamburger on the left, Title in center, Bell on the right */}
      <header className="w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-13 sm:h-14 flex items-center justify-between">
        ...
        >
          ...
          {hasWeeklyTriggerActive ? (
            <span className="absolute top-2 right-2 flex h-3 w-3">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-3 w-3 bg-amber-400 border-2 border-[#1565ff]"></span>
            </span>
          ) : (

...
import { 
  ...
  isMondaySummaryDismissed, 
  dismissMondaySummary, 
  resetMondaySummaryDismissal 
} from '../utils/weeklyNotificationTrigger';
import { MondayWeeklySummaryCard } from './MondayWeeklySummaryCard';
...
export const RiskDashboard: React.FC<RiskDashboardProps> = ({
  ...
  // Monday Morning Performance Trigger State
  const [forceSimulateMonday, setForceSimulateMonday] = useState<boolean>(() => getIsMondaySimulationActive());
  const [isDismissedManual, setIsDismissedManual] = useState<boolean>(() => isMondaySummaryDismissed());
  ...
  }, [trades, openPositions, stats?.startingBalance, plan?.startingCapital, forceSimulateMonday]);

  const handleToggleMondaySimulation = () => {
    const next = !forceSimulateMonday;
    setForceSimulateMonday(next);
    setMondaySimulationActive(next);
    if (next) {
      resetMondaySummaryDismissal();
  ...
  const handleDismissMondaySummary = () => {
    dismissMondaySummary();
  ...
  return (
    <div id="risk-dashboard-container" className="space-y-4 max-w-xl sm:max-w-2xl mx-auto pb-8">
      ...
      {/* 1. MONDAY MORNING PERFORMANCE TRIGGER BRIEFING */}
      {weeklySummary.isTriggered && !isDismissedManual && (
        <MondayWeeklySummaryCard
          summary={weeklySummary}
          onDismiss={handleDismissMondaySummary}
          onNavigateTab={onNavigateTab}
          onToggleSimulation={handleToggleMondaySimulation}
        />
      )}

      {/* Standby Status Pill for Monday Trigger when inactive or dismissed */}
      {(!weeklySummary.isTriggered || isDismissedManual) && (
        <div 
          id="monday-trigger-standby-pill"
          className="flex items-center justify-between px-3 py-2 bg-white border border-slate-200/90 rounded-xl text-xs text-slate-600 shadow-2xs"
        >
          <div className="flex items-center gap-2 min-w-0">
            <span className={`w-2 h-2 rounded-full shrink-0 ${
              weeklySummary.isRealMondayMorning && weeklySummary.hasActiveTrades 
                ? 'bg-emerald-500 animate-pulse' 
                : 'bg-[#1565ff]'
            }`} />
            <span className="font-semibold text-slate-800 truncate">
              Monday Performance Trigger:
            </span>
            <span className="text-slate-500 text-[11px] truncate hidden sm:inline">
              {weeklySummary.isRealMondayMorning 
                ? (weeklySummary.hasActiveTrades ? 'Active on Monday Morning' : 'Waiting for active trades')
                : 'Fires every Monday AM when active trades exist'}
            </span>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              type="button"
              id="test-monday-trigger-btn"
              onClick={handleToggleMondaySimulation}
              className="text-xs font-bold text-[#1565ff] hover:text-[#0c53dc] hover:underline cursor-pointer flex items-center gap-1"
            >
              {isDismissedManual ? 'Reopen Briefing' : 'Test Trigger'}
            </button>
          </div>
        </div>
      )}

...
Est. Liquidation: $${formatNumber(result.liquidationPrice)} (${result.distanceToLiquidationPct.toFixed(2)}% distance)`;
  ...
  const handleTrackInPortfolio = async () => {
    ...
    const isDex = tradingMode === 'SPOT' && spotVenue === 'DEX';
    const effectiveLev = tradingMode === 'SPOT' ? 1 : tradingMode === 'FOREX' ? (calcState.forexLeverage || 100) : calcState.leverage;
    ...
    const newPos: OpenPosition = {
      ...
      margin: result.marginRequired || (effectiveLev > 0 ? result.positionSize / effectiveLev : result.positionSize),
      ...
      distanceToLiquidationPct: result.distanceToLiquidationPct,
      correlationGroup: 
        tradingMode === 'FOREX' ? 'Forex Majors' :
        calcState.pair.startsWith('BTC') ? 'BTC-Related' :
        calcState.pair.startsWith('ETH') ? 'Layer-1s' :
        spotVenue === 'DEX' ? 'Memes & High-Beta' : 'Altcoins',
      exchange: isDex ? 'Other' : ((calcState.exchange as any) || 'Binance'),
      createdAt: new Date().toISOString(),
      lotSize: result.forex?.lotSize,
      pipSize: calcState.forexPipSize,
      pipValue: calcState.forexPipValue,
      dexChain: isDex ? (calcState.chain || 'Ethereum') : undefined,
      dexProtocol: isDex ? (calcState.dex || 'Uniswap') : undefined,
      dexToken: isDex ? (calcState.token || calcState.pair) : undefined,
          ...
          <div id="calculator-action-buttons" className="space-y-2">
            ...
            {positionTrackedSuccess && (
              <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center justify-between text-xs text-emerald-800 animate-in fade-in">
                <span className="font-semibold">✓ Position added to active portfolio risk monitor!</span>
                {onNavigateToPortfolio && (
                  <button
                    type="button"

...
export const LeverageSimulator: React.FC<LeverageSimulatorProps> = ({
  ...
  stopLossPrice: propStopLossPrice,
  direction: propDirection,
  currentLeverage: propCurrentLeverage,
}) => {
  const [direction, setDirection] = useState<TradeDirection>(propDirection || initialState?.direction || 'LONG');
  const [entryPrice, setEntryPrice] = useState<number>(propEntryPrice || initialState?.entryPrice || 60000);
  const [stopLoss, setStopLoss] = useState<number>(propStopLossPrice || initialState?.stopLoss || (initialState?.direction === 'SHORT' ? 61200 : 58800));
  const [takeProfit, setTakeProfit] = useState<number>(initialState?.takeProfit || (initialState?.direction === 'SHORT' ? 57000 : 63000));
  const [simLeverage, setSimLeverage] = useState<number>(propCurrentLeverage || initialState?.leverage || 10);

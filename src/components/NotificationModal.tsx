...
export const NotificationModal: React.FC<NotificationModalProps> = ({
  ...
  const notifications = [
    ...(alerts.length > 0 ? [{
      id: 'price-alerts',
      title: triggeredAlerts.length > 0 
        ? `${triggeredAlerts.length} Price Alert(s) Triggered!`
        : `${activeAlerts.length} Price Alert(s) Armed & Monitoring`,
      desc: triggeredAlerts.length > 0
        ? `Alert triggered for ${triggeredAlerts.map(a => `${a.pair} (${a.targetType})`).join(', ')}. Target level approached.`
        : `Monitoring live feeds for ${activeAlerts.map(a => a.pair).slice(0, 3).join(', ')}${activeAlerts.length > 3 ? '...' : ''} with browser notifications and audio chimes.`,
      time: triggeredAlerts.length > 0 ? 'Triggered' : 'Live Feeds',
      type: (triggeredAlerts.length > 0 ? 'warning' : 'info') as 'warning' | 'info' | 'danger' | 'success',
      icon: Bell,
      color: triggeredAlerts.length > 0
        ? 'text-amber-700 bg-amber-50 border-amber-300 ring-1 ring-amber-300/50'
        : 'text-[#1565ff] bg-blue-50 border-blue-200',
      action: onNavigateToCalculator,
      actionLabel: 'Open Calculator',
    }] : []),
    {
      id: 'monday-weekly-summary',
      title: weeklySummary?.isTriggered 
        ? "Monday Weekly Performance Briefing"
        : "Monday Morning Performance Trigger (Armed)",
      desc: weeklySummary?.hasActiveTrades
        ? `Week Summary: ${weeklySummary.winRate}% Win Rate, ${weeklySummary.totalPnl >= 0 ? '+' : ''}$${weeklySummary.totalPnl.toFixed(2)} Total PnL across ${weeklySummary.totalTrades} closed trades with ${weeklySummary.activeTradesCount} active positions.`
        : `Armed: Displays a weekly summary of Win Rate & Total PnL on the dashboard every Monday morning if active trades are running.`,
      time: weeklySummary?.isTriggered ? 'Triggered' : 'Every Mon AM',
      type: weeklySummary?.isTriggered ? 'success' : 'info',
      icon: Calendar,
      color: weeklySummary?.isTriggered 
        ? 'text-blue-900 bg-blue-50/90 border-blue-300 ring-1 ring-blue-300/50' 
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      {n.action && (
                        <button
                          ...
                          className="px-2.5 py-1 bg-[#1565ff] hover:bg-[#0c53dc] text-white text-[11px] font-bold rounded-lg transition-colors flex items-center gap-1 cursor-pointer"
                        >
                          <span>{n.actionLabel || 'View'}</span>
                          <ArrowRight className="w-2.5 h-2.5" />
                        </button>
                      )}
                      {n.hasSimToggle && onToggleMondaySimulation && (
                        <button
                          type="button"
                          onClick={onToggleMondaySimulation}
                          className="px-2 py-1 bg-white hover:bg-slate-100 text-slate-700 text-[11px] font-semibold rounded-lg border border-slate-200 transition-colors cursor-pointer"
                        >
                          {weeklySummary?.isSimulated ? 'Exit Sim' : 'Test Sim'}
                        </button>
                      )}
                    </div>
                  )}

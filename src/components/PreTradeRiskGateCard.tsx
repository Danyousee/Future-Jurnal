    ...
    >
      ...
      {showRiskDetails && (
        <div className="mt-3 pt-3 border-t border-slate-100 animate-in fade-in duration-150 space-y-2">
          ...
          {/* Active Portfolio Risk Integration Row */}
          {metrics.maxPortfolioRiskBudget !== undefined && (
            <div className="p-3 bg-blue-50/50 rounded-xl border border-blue-100 space-y-2 mt-2">
              <div className="flex items-center justify-between text-xs">
                <span className="font-bold text-slate-800 flex items-center gap-1.5">
                  ...
                  Active Portfolio Risk & Capacity:
                </span>
                <span className="font-mono font-bold text-slate-600 text-[11px]">
                  {metrics.openPositionsCount || 0} Open Position{(metrics.openPositionsCount || 0) !== 1 ? 's' : ''}
                </span>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                <div className="bg-white p-2 rounded-lg border border-slate-200">
                  <span className="text-[10px] text-slate-500 font-medium block">Current Open Risk</span>
                  <span className="font-mono font-bold text-slate-800">{formatCurrency(metrics.currentOpenRisk || 0)}</span>
                </div>
                <div className="bg-white p-2 rounded-lg border border-slate-200">
                  <span className="text-[10px] text-slate-500 font-medium block">+ New Trade Risk</span>
                  <span className="font-mono font-bold text-[#1565ff]">+{formatCurrency(result.riskAmount)}</span>
                </div>
                <div className="bg-white p-2 rounded-lg border border-slate-200">
                  <span className="text-[10px] text-slate-500 font-medium block">Prospective Total</span>
                  <span className={`font-mono font-bold ${(metrics.prospectiveTotalRisk || 0) > (metrics.maxPortfolioRiskBudget || 0) ? 'text-[#ff3b4a]' : 'text-slate-900'}`}>
                    {formatCurrency(metrics.prospectiveTotalRisk || 0)}
                  </span>
                </div>
                <div className="bg-white p-2 rounded-lg border border-slate-200">
                  <span className="text-[10px] text-slate-500 font-medium block">Remaining Capacity</span>
                  <span className={`font-mono font-bold ${(metrics.remainingPortfolioRiskCapacity || 0) > 0 ? 'text-[#22a65e]' : 'text-[#ff3b4a]'}`}>
                    {formatCurrency(metrics.remainingPortfolioRiskCapacity || 0)}
                  </span>
                </div>
              </div>
            </div>

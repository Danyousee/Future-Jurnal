...
export type Exchange = 'Binance' | 'Bybit' | 'OKX' | 'Kucoin' | 'Bitget' | 'Coinbase' | 'Other' | (string & {});
...
// Multi-Market Trading Modes - Exactly 3 Primary Modes
export type TradingMode = 'SPOT' | 'PERPETUAL' | 'FOREX';
export type PrimaryTradingMode = 'SPOT' | 'PERPETUAL' | 'FOREX';
export type SpotVenue = 'CEX' | 'DEX';
export type DexChain = 'Ethereum' | 'BNB Chain' | 'Solana' | 'Arbitrum' | 'Base' | 'Polygon' | (string & {});
export type SpotDirection = 'BUY' | 'SELL';
export type SpotSizingMethod = 'RISK_BASED' | 'CAPITAL_BASED';

export interface SpotCalculatorState {
  venue: SpotVenue;
  pair: string;
  ...
  feeRate: number;
  slippagePct: number;
  // DEX execution parameters (used when venue === 'DEX')
  chain?: string;
  dex?: string;
  token?: string;
...
export interface ForexCalculatorState {
  pair: string;
...
export interface DexCalculatorState {
  pair: string;
  token: string;
  chain: string;
  dex: string;
...
export interface CalculatorState {
  ...
  // Spot mode specific state & inputs
  spotVenue?: SpotVenue;
  spotDirection?: SpotDirection;
  spotMaxCapital?: number;
  spotSizingMethod?: SpotSizingMethod;
  spotState?: Partial<SpotCalculatorState>;
  tokenLiquidity?: number;

  // Forex mode specific state & inputs
  forexAccountCurrency?: string;
  forexLeverage?: number;
  forexPipSize?: number;
  forexPipValue?: number;
  forexStandardLotUnits?: number;
  forexState?: Partial<ForexCalculatorState>;

  // DEX mode specific state & inputs (when spotVenue === 'DEX')
  dexToken?: string;
  dexChain?: string;
  dexProtocol?: string;
  dexWallet?: string;
  ...
  dexTokenLiquidity?: number;
  dexState?: Partial<DexCalculatorState>;
}
...
export interface CalculationResult {
  ...
  positionSize: number; // Notional USDT or total position value
  quantity: number; // Base crypto asset amount or currency units
  riskAmount: number;
  reward: number;
  ...
  // Spot-specific details (Shared by CEX and DEX)
  spot?: {
    venue: SpotVenue;
    capitalRequired: number;
    capitalLimitReached: boolean;
    sizingMethod: SpotSizingMethod;
    maxCapital: number;
    riskPerUnit: number;
...
export interface TradeJournalEntry {
  ...
  time?: string; // HH:mm
  tradeMode?: TradingMode | 'DEX'; // Primary: 'SPOT' | 'PERPETUAL' | 'FOREX', legacy 'DEX' for backward compatibility
  pair: string; // e.g. BTCUSDT, ETHUSDT, EUR/USD
  exchange: Exchange;
  entryPrice: number;
  exitPrice: number;
  positionSize: number; // Notional USDT / Position Value
  quantity?: number; // Contracts / Coins / Units
  ...
  riskAmount?: number;
  riskPercentage?: number;
  // Multi-market specific details
  spotVenue?: SpotVenue; // 'CEX' | 'DEX' when tradeMode === 'SPOT'
  lotSize?: number;
  pipsRisk?: number;
  pipsGain?: number;
  capitalRequired?: number;
...
export interface PreTradeValidationResult {
  ...
  metrics: {
    ...
    prospectiveTotalRisk?: number;
    maxPortfolioRiskBudget?: number;
    remainingPortfolioRiskCapacity?: number;
    prospectiveMarginUtilizationPct?: number;
    correlatedGroupRisk?: number;
    correlatedGroupName?: string;
  };
}
...
export interface OpenPosition {
  ...
  stopLoss: number;
  takeProfit?: number;
  quantity: number; // Base crypto asset count / lots
  positionSize: number; // Notional USDT / currency
  margin: number; // Margin locked / capital allocated
  leverage: number;
  ...
  liquidationPrice?: number;
  distanceToLiquidationPct?: number;
  correlationGroup?: string; // e.g. 'BTC-Related', 'Layer-1s', 'High-Beta DeFi', 'Memes', 'Forex Majors', 'Custom'
  exchange?: Exchange;
  wallet?: string;
  dexChain?: string;
  dexProtocol?: string;
  ...
  pipValue?: number;
  feesPaid?: number;
  positionStatus?: 'OPEN' | 'APPROACHING_SL' | 'SL_HIT' | 'APPROACHING_TP' | 'TP_HIT' | 'CLOSED';
  latestPriceTimestamp?: number;
  priceSource?: string;
  isPriceLive?: boolean;
...
export type PositionStatus = 'OPEN' | 'APPROACHING_SL' | 'SL_HIT' | 'APPROACHING_TP' | 'TP_HIT' | 'CLOSED';
...
export interface CorrelatedGroupRisk {
  groupName: string;
  exposureRisk: number;
  exposureRiskPct: number;
...
export interface PortfolioRiskSummary {
  ...
  portfolioRiskTier: 'CONSERVATIVE' | 'MODERATE' | 'AGGRESSIVE' | 'CRITICAL';
  riskWarnings: string[];
  correlatedGroups: CorrelatedGroupRisk[];
  totalPortfolioEquity: number;
  maxAllowedPortfolioRisk: number;
  maxAllowedPortfolioRiskPct: number;
  remainingRiskCapacity: number;
  remainingRiskCapacityPct: number;
  winningCount?: number;
  losingCount?: number;
  approachingSlCount?: number;
  approachingTpCount?: number;
  slHitCount?: number;
  tpHitCount?: number;
...
export type PriceAlertTargetType = 'ENTRY' | 'STOP_LOSS' | 'TAKE_PROFIT' | 'CUSTOM';
export type PriceAlertCondition = 'APPROACHING' | 'AT_OR_ABOVE' | 'AT_OR_BELOW';
...
export interface PriceAlert {
  ...
  targetPrice: number;
  initialPrice?: number;
  proximityPct: number; // e.g. 0.5 means triggers within 0.5% of target price
